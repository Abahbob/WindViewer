...
namespace WWActorEdit
{
    public class ZeldaArc
    {
        ...
        {
            Translation = trans;
            foreach (DZB D in DZBs) D.ChangeTranslation(Translation);
            foreach (J3Dx J in J3Dxs) J.ChangeTranslation(Translation, GlobalRotation);
        }
        ...
        public void ChangeRotation(Vector3 rot)
        {
            ...
            foreach (DZB D in DZBs) Center = D.ChangeRotation(Rotation);
            foreach (J3Dx J in J3Dxs) J.ChangeTranslation(Translation, GlobalRotation, Rotation);
        }
        ...
        public void Save()
        {
            foreach (DZx D in DZRs)
            {
                ...
                foreach (DZx.FileChunk C in D.Chunks)
                    foreach (IDZxChunkElement CE in C.Data)
                        if (CE.HasChanged == true || Translation != new Vector3(0,0,0)) CE.StoreChanges();
            ...
            {
                if (D.ParentFile.IsCompressed == true) continue;
            ...
            {
                if (J.FileEntry.IsCompressed == true) continue;
        ...
        public void ShowControl(Panel Parent)
        {
            ...
            EditControl = new ZAControl(this);
